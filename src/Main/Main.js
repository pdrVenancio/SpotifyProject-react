//O useState é um Hook que permite adicionar estado a componentes funcionais em React
//O useEffect é um Hook que lida com efeitos colaterais em componentes funcionais
...
const Main = ({ searchInput }) => {
    // Estado para armazenar os dados dos artistas vindos da API
    const [artistData, setArtistData] = useState([])
    
    // Estado para controlar a visibilidade do resultado da playlist
    const [resultPlaylistVisible, setResultPlaylistVisible] = useState(true)

    // O useEffect é uma função especial que é executada sempre que o componente é 
    // renderizado ou quando dependências mudam
    useEffect(() => {
        if (searchInput === '') {
            setResultPlaylistVisible(true)
            return
        }
    ...
    function requestApi(searchTerm) {
        //json-server-plus --watch src\api-artists\artists.json --port 3001
        ...
        fetch(url)
            ...
            .then((result) => {
                ...
                setResultPlaylistVisible(false)
            })
    }

    return (
        <div className="playlist-container">
            <div id="result-playlists" className={resultPlaylistVisible ? '' : 'hidden'}>
            ...
            <div id="result-artist" className={resultPlaylistVisible ? 'hidden' : ''}>
